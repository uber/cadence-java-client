...
public class TracingPropagator {
  // span names
  private static final String EXECUTE_WORKFLOW = "ExecuteWorkflow";
  private static final String EXECUTE_ACTIVITY = "ExecuteActivity";
  private static final String EXECUTE_LOCAL_ACTIVITY = "ExecuteLocalActivity";
  ...
  private SpanContext extract(Map<String, byte[]> headers) {
    return tracer.extract(Format.Builtin.TEXT_MAP, new TextMapAdapter(
  ...
  private SpanContext extract(Header header) {
    return tracer.extract(Format.Builtin.TEXT_MAP, new TextMapAdapter(
      header.getFields().entrySet().stream().collect(Collectors.toMap(
      Map.Entry::getKey, e -> new String(e.getValue().array())
    ))));
  }
}
